...
import {UserCard, UserCardContainer} from '../components/UserCard'
...
export default function FollowingPage (){
    const {shownUser, followingUsers, setShownUser, setFollowingUsers} = useContext(GlobalContext)
    ...
    const checkUser = async(login) =>{
        try{
            const checkedUser = await axios.get(`${BASE_URL}/users/${login}`)
            setShownUser(checkedUser.data)
            goToProfile(history, checkedUser.data.login)
        }catch(error){
            alert(error.response.data.message)
        }
    }
